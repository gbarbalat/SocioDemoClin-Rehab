SL.library <- list("SL.mean",
                   c("SL.caret.xgboost", "All",    "screen.randomForest"),
                   c("SL.caret.ranger", "All",   "screen.randomForest"),
                   #c("SL.caret.earth", "All",  "screen.randomForest"),
                   #c("SL.caret.glm", "All",    "screen.randomForest"),
                   c("SL.glm", "All",    "screen.randomForest"),
                   c("SL.caret.step.interaction",    "screen.randomForest"),#,
                   c("SL.caret.glmnet", "All",    "screen.randomForest")
)
#SL.library <- c("SL.mean","SL.glm","SL.caret.ranger")
                          ...
                          metric = ifelse(family$family == "gaussian", "RMSE", "logLoss"), ...) 
{
  ...
  if (family$family == "gaussian") {
    fit.train <- caret::train(x = X, y = Y, weights = obsWeights, 
                              metric = metric, method = method, tuneLength = tuneLength, 
                              trControl = trControl, 
                              direction = "both", trace = 0, k = 2,scope = Y ~ .^2
    )
    pred <- predict(fit.train, newdata = newX, type = "raw")
  }
  if (family$family == "binomial") {
